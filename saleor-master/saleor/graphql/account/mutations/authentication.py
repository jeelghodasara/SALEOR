...
class SocialCreateToken(BaseMutation):
    ...
    class Arguments:
        email = graphene.String(required=True, description="Email of a user.")
        googleId = graphene.String(required=True, description="GoogleId of a user.")
    ...
    @classmethod
    def _retrieve_user_from_credentials(cls, email, googleId) -> Optional[models.User]:
        if email and googleId:
            user = models.User.objects.filter(
                email=email, is_active=True, googleId=googleId).first()
            if user:
                return user
        return None
    ...
    @classmethod
    def get_user(cls, _info, data):
        user = cls._retrieve_user_from_credentials(data["email"], data["googleId"])
        if not user:
            raise ValidationError(
                {
                    "email": ValidationError(
